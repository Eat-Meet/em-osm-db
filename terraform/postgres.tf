  ...
  body = <<-EOT
    BEGIN
        ...
        SELECT DISTINCT
            ...
            p.tags
        FROM
            ${postgresql_schema.lviv_schema.name}.planet_osm_point p
        JOIN
            ${postgresql_schema.lviv_schema.name}.planet_osm_polygon poly ON ST_Contains(poly.way, p.way)
        WHERE
  ...
  body = <<-EOT
    BEGIN
        ...
        SELECT DISTINCT
            ...
            p.tags
        FROM
            ${postgresql_schema.kyiv_schema.name}.planet_osm_point p
        JOIN
            ${postgresql_schema.kyiv_schema.name}.planet_osm_polygon poly ON ST_Contains(poly.way, p.way)
        WHERE
            p.amenity = ANY(amenities)
            AND p.name IS NOT NULL
            and poly.name = 'Київ'
